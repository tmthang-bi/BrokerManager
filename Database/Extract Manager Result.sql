...
FROM BrokerCalculationResult r INNER JOIN Broker b ON r.BrokerID = b.ID LEFT OUTER JOIN Broker b2 on b.SupervisorBrokerID = b2.ID INNER JOIN Office o ON b.OfficeID = o.ID INNER JOIN OfficeCalculationResult ofr ON b.OfficeID = ofr.OfficeID WHERE (r.PeriodId = '201601') AND (b.BrokerTypeID = 'MAN') AND (ofr.PeriodId = '201601') ORDER BY ID